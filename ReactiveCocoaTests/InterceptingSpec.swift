...
fileprivate struct ReturnValueTest<I: Equatable> {
	...
	func call(offset: UInt) {
		switch I.self {
		case is CChar.Type:
			let arg = argument(forOffset: offset) as! CChar
	...
	override func spec() {
		...
		describe("two classes in the same hierarchy") {
			...
			it("should not collide") {
				...
				let expectedSuperValues = [40, "foo", "Not Subclass 40 foo"] as NSArray
				expect(superclassTuple as NSArray) == expectedSuperValues

				expect(subclassObj.foo(40, "foo")) == "Subclass 40 foo"

				let expectedSubValues = [40, "foo", "Subclass 40 foo"] as NSArray
				expect(subclassTuple as NSArray) == expectedSubValues
			}
